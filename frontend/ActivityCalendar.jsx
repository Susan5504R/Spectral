// frontend/ActivityCalendar.jsx
// Drop this into any React project. Fetches from your /activity/:userId endpoint.
...
function groupByWeek(activity) {
    const weeks = [];
    let week = [];
    // Pad so first day aligns to correct column (Sunday = 0)
    if (activity.length > 0) {
        const firstDay = new Date(activity[0].date + "T00:00:00Z").getUTCDay();
        for (let p = 0; p < firstDay; p++) week.push(null); // empty padding cells
    }
...
export default function ActivityCalendar({ userId, apiBase = "http://localhost:5000" }) {
...
function Cell({ day, size, onEnter, onLeave, onMove }) {
    ...
    return (
        <div
            ...
            onMouseOver={e => { e.currentTarget.style.filter = "brightness(1.4)"; e.currentTarget.style.transform = "scale(1.3)"; }}
