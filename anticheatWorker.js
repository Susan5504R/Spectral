...
const { initDb, ASTFingerprint, PlagiarismCheck, Submission } = require("./db");
const { cosineSim, jaccard, getFingerprints } = require("./anticheat/winnow");

const REDIS_HOST = process.env.REDIS_HOST || "127.0.0.1";

async function startAnticheatWorker() {
    await initDb({ logPrefix: "ANTICHEAT" });
...
startAnticheatWorker().catch(err => {
    console.error("[ANTICHEAT] Failed to start:", err.message);
    process.exit(1);
});
