...
app.get("/me/profile", authenticateToken, async (req, res) => {
    try {
        const user = await User.findByPk(req.user.id, {
            attributes: ["id", "username", "createdAt", "bio", "avatarUrl"]
        });
...
app.put("/me/profile", authenticateToken, async (req, res) => {
    try {
        ...
        // Create an update object based on what is sent
        const updateData = {};
        if (username !== undefined) updateData.username = username;
        
        // We use 'null' or empty string checks to allow users to clear their bio/avatar
        updateData.bio = bio; 
